...
async def register_user(payload: UserRegisterRequest) -> TokenResponse:
    """
    Onboards a new student or software engineer, saves track preference,
    ...
    logger.info(
        f"User successfully registered: {clean_email} [Track: {payload.learning_track}]",
...
async def login_user(payload: UserLoginRequest) -> TokenResponse:
    ...
    Validates operational analyst credentials and newly registered users.
    """
    clean_email = payload.email.strip().lower()
