...
export default function AuthCard({ 
  ...
  // Handle Instant Guest Access: Blocked from dashboard
  const handleGuestAccess = () => {
    setFeedback({
      type: 'error',
      message: lang === 'en'
        ? 'Access Restricted: Guest bypass is disabled. Please authenticate with an authorized Operator ID (@siliconbrain.ai).'
        : 'యాక్సెస్ పరిమితం: గెస్ట్ యాక్సెస్ నిలిపివేయబడింది. దయచేసి అధికారిక ఆపరేటర్ ID (@siliconbrain.ai) తో లాగిన్ అవ్వండి.'
    });
  };

  // Handle Google SSO: Strict security block - NO dashboard access
  const handleGoogleSSO = () => {
    setFeedback({
      type: 'error',
      message: lang === 'en'
        ? 'Access Denied: Google SSO cannot access this dashboard. Only Corporate Operator IDs (@siliconbrain.ai) are authorized.'
        : 'యాక్సెస్ నిరాకరించబడింది: Google SSO కు డ్యాష్‌బోర్డ్ అనుమతి లేదు. కేవలం కార్పొరేట్ ఆపరేటర్ ఖాతాలు (@siliconbrain.ai) మాత్రమే చెల్లుతాయి.'
    });
  };
    ...
    <div className="relative rounded-3xl bg-slate-900/85 border border-cyan-500/30 p-6 sm:p-7 backdrop-blur-2xl shadow-2xl shadow-cyan-950/40 overflow-hidden flex flex-col justify-between">
      ...
      <div className="relative z-10 grid grid-cols-2 gap-2.5 mb-4">
        ...
        <button
          ...
          title="Google SSO is disabled. Dashboard is restricted to verified Corporate Operator IDs (@siliconbrain.ai)"
          className="flex items-center justify-center space-x-1.5 py-2.5 px-3 rounded-xl bg-slate-950/40 hover:bg-slate-900 border border-slate-800/80 text-xs font-semibold text-slate-400 hover:text-slate-300 transition-all active:scale-95 group shadow-sm opacity-80"
        >
          ...
          <span className="truncate">{landingT.ssoGoogle || "Google SSO"}</span>
          <span className="text-[9px] font-mono px-1 rounded bg-rose-500/20 text-rose-400">Lock</span>
        </button>
      </div>
